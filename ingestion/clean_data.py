import pandas as pd
from psycopg2 import sql
from database.db_connection import connect_postgres   # ✅ reuse your connection function
from dotenv import load_dotenv
import traceback

import os
load_dotenv()


def clean_and_transform():
    host=os.getenv("host")
    dbname=os.getenv("dbname")
    user=os.getenv("user")
    password=os.getenv("password")
    port=5432
    try:
        # Connect to DB
        conn = connect_postgres(host,dbname,user,password,port)
        cursor = conn.cursor()
        print("✅ Connected to PostgreSQL")

        # Step 1: Read raw data from staging table
        query = "SELECT * FROM train_delays_raw;"
        df = pd.read_sql(query, conn)

        # Step 2: Clean column names to match final schema
        df.rename(columns={
            "distance(Km)": "distance_km",
            "sc_arr__time": "sc_arr_time",
            "dealy_min": "delay_min",
        }, inplace=True)

        # Step 3: Convert date column (DD-MM-YYYY HH:MM → TIMESTAMP)
        df['date'] = pd.to_datetime(df['date'], format="%d-%m-%Y %H:%M")

        # Step 4: Convert time columns (HH:MM:SS → TIME)
        df['sc_arr_time'] = pd.to_datetime(df['sc_arr_time'], format="%H:%M:%S").dt.time
        df['act_arr_time'] = pd.to_datetime(df['act_arr_time'], format="%H:%M:%S").dt.time
        df['delay_min'] = pd.to_datetime(df['delay_min'], format="%H:%M:%S").dt.time

        # Create table (adjust schema to match your CSV)
        cursor.execute("""
        CREATE TABLE IF NOT EXISTS train_delays_cleaned (
            train_id VARCHAR(50),
            train_name VARCHAR(100),
            train_no INT,
            source VARCHAR(100),
            destination VARCHAR(100),
            date TIMESTAMP,
            "distance_km" INT,
            sc_arr_time TIME,
            act_arr_time TIME,   
            delay_min TIME,
            season VARCHAR(50),
            run_frequency VARCHAR(50)   
            
        );
        """)
        conn.commit()
        print("📌 Table created successfully")

        # Step 5: Insert cleaned data into final table
        for _, row in df.iterrows():
            cursor.execute(
                sql.SQL("""
                    INSERT INTO train_delays_cleaned (
                        train_id, train_name, train_no, source, destination, date,
                        "distance_km", sc_arr_time, act_arr_time, delay_min,
                        season, run_frequency
                    )
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                """),
                (
                    row['train_id'], row['train_name'], row['train_no'],
                    row['source'], row['destination'], row['date'],
                    row['distance_km'], row['sc_arr_time'], row['act_arr_time'],
                    row['delay_min'], row['season'], row['run_frequency']
                )
            )

        conn.commit()
        print("🚂 Cleaned data loaded into train_delays_cleaned successfully!")

        cursor.close()
        conn.close()
    except Exception as e:
        print("❌ Error:", e)
        traceback.print_exc() 


# if __name__ == "__main__":
#     clean_and_transform()
